...
# coq_makefile -R . X86Model -R ../../shared Shared X86Syntax.v ParserArg.v Xform.v Regexp.v RESet.v GrammarType.v Grammar.v BiGrammar.v Parser.v Recognizer.v X86BG_ins.v X86BG.v Encode.v X86Semantics.v SemTests.v Decode.v -o Makefile.coq 
#
...
install:
	cd "." && for i in $(VOFILES) $(VFILES) $(GLOBFILES) $(NATIVEFILES) $(CMOFILES) $(CMIFILES) $(CMAFILES); do \
	 install -d "`dirname "$(DSTROOT)"$(COQLIBINSTALL)/X86Model/$$i`"; \
	 install -m 0644 $$i "$(DSTROOT)"$(COQLIBINSTALL)/X86Model/$$i; \
	done
...
uninstall_me.sh: Makefile.coq
	echo '#!/bin/sh' > $@
	printf 'cd "$${DSTROOT}"$(COQLIBINSTALL)/X86Model && rm -f $(VOFILES) $(VFILES) $(GLOBFILES) $(NATIVEFILES) $(CMOFILES) $(CMIFILES) $(CMAFILES) && find . -type d -and -empty -delete\ncd "$${DSTROOT}"$(COQLIBINSTALL) && find "X86Model" -maxdepth 0 -and -empty -exec rmdir -p \{\} \;\n' >> "$@"
