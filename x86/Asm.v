...
Definition parent_pointer (m: mem) : option block :=
  match Mem.stack_adt m with
  | _::(b::nil,_,_)::_ => Some b 
  | _ => None
  end.
...
Definition exec_instr {exec_load exec_store} `{!MemAccessors exec_load exec_store} {F V} (ge: Genv.t F V) (f: function) (i: instruction) (rs: regset) (m: mem) : outcome :=
  ...
  | Pfreeframe sz ofs_ra (* ofs_link *) =>
      ...
      | Some ra =>
          ...
          | Some sp =>
              match rs#RSP with
              | Vptr stk ofs =>
                if check_top_frame m stk sz (Vptr sp Ptrofs.zero) (* ofs_link *) (* ofs_ra *)
                then
                  match Mem.free m stk 0 sz with
                  | None => Stuck
                  | Some m' =>
                    match Mem.unrecord_stack_block m' with
                      Some m' => Next (nextinstr (rs#RSP <- (Vptr sp Ptrofs.zero) #RA <- ra)) m'
                    | None => Stuck
                    end
                  end
                else Stuck
  ...
  | Pload_parent_pointer rd =>
    match parent_pointer m with
    | Some b => Next (nextinstr (rs#rd <- (Vptr b Ptrofs.zero))) m
    | None => Next (nextinstr (rs#rd <- init_sp)) m
    end
