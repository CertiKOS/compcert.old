...
(* CompCert assembly adapted to a flat memory *)
(* Author: Yuting Wang *)
(* Date  : 10-9-2017 *)
...
Inductive instruction: Type :=
  ...
  | Pjcc (c: testcond)(l: ptrofs)
  | Pjcc2 (c1 c2: testcond)(l: ptrofs)   (**r pseudo *)
  | Pjmptbl (r: ireg) (tbl: list ptrofs) (**r pseudo *)
...
Definition inst_size (i:instruction) : positive := 8%positive.


(** ** Mapping from flat memory addresses *)
Module IndexedPtrofs <: EQUALITY_TYPE.

Definition t := ptrofs.
Definition eq := Ptrofs.eq_dec.

End IndexedPtrofs.

Module PtrofsMap := EMap(IndexedPtrofs).


(** code is a list of memory addresses to instructions *)
Definition code := list ptrofs.
...
- a list of definitions of functions;
...
- the identifer of the block representing the flat memory 
- the range of the memory section for global variables 
- the range of the memory section for the stack 
- the range of the memory section for code
- the mapping from memory addresses to instructions in the code section
*)
...
Record program : Type := {
  ...
  code_start  : ptrofs;
  code_end    : ptrofs;
  code_mapping : PtrofsMap.t instruction;
}.
...
Definition make_program 
           (defs: list (ident * option (globdef fundef unit)))
           (public: list ident)
           (main: ident) 
           (fm_block : block)
           (size : ptrofs)
           (g_start g_end : ptrofs) 
           (s_start s_end : ptrofs)
           (c_start c_end : ptrofs)
           (cmapping : PtrofsMap.t instruction): program :=
  {| prog_defs := defs;
     prog_public := public;
     prog_main := main;
     flat_mem_block := fm_block;
     mem_size := size;
     globs_start := g_start;
     globs_end := g_end;
     stack_start := s_start;
...
Record genv : Type := mkgenv {
  ...
  genv_defs: PTree.t (globdef fundef unit);     (**r mapping block -> definition *)
  ...
  (** Information about the flat memory *)
  fm_block : block;
  fm_size  : ptrofs;
  gb_start : ptrofs;
  gb_end   : ptrofs;
  ...
  cd_start  : ptrofs;
  cd_end    : ptrofs;
  cd_mapping : PtrofsMap.t instruction;  
}.

Definition genv_of_genv (p: genv) : Genv.t fundef unit :=
  {|  
    Genv.genv_public := p.(genv_public);
    Genv.genv_symb   := p.(genv_symb);
    Genv.genv_defs   := p.(genv_defs);
    Genv.genv_next   := p.(genv_next);
    Genv.genv_symb_range := p.(genv_symb_range);
    Genv.genv_defs_range := p.(genv_defs_range);
    Genv.genv_vars_inj   := p.(genv_vars_inj);
  |}.

Coercion genv_of_genv: genv >-> Genv.t.
