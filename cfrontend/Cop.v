...
Definition sem_cast (v: val) (t1 t2: type) (m: T): option val :=
  ...
  | cast_case_p2bool =>
      ...
      | Vptr b ofs => if weak_valid_pointer m b (Int.unsigned ofs) then Some Vone else None
      | _ => None
      end
  | cast_case_l2l =>
      match v with
...
Definition bool_val (v: val) (t: type) (m: T) : option bool :=
  ...
  | bool_case_p =>
      match v with
      | Vint n => Some (negb (Int.eq n Int.zero))
      | Vptr b ofs => if weak_valid_pointer m b (Int.unsigned ofs) then Some true else None
      | _ => None
      end
  | bool_case_l =>
      match v with
...
Definition sem_notbool (v: val) (ty: type) (m: T): option val :=
  ...
  | bool_case_p =>
      match v with
      | Vint n => Some (Val.of_bool (Int.eq n Int.zero))
      | Vptr b ofs => if weak_valid_pointer m b (Int.unsigned ofs) then Some Vfalse else None
      | _ => None
      end
  | bool_case_l =>
      match v with
...
Definition sem_cmp (c:comparison)
  ...
  | cmp_case_pp =>
      option_map Val.of_bool (Val.cmpu_bool (valid_pointer m) c v1 v2)
  | cmp_case_pl =>
      match v2 with
      | Vlong n2 =>
          let n2 := Int.repr (Int64.unsigned n2) in
          option_map Val.of_bool (Val.cmpu_bool (valid_pointer m) c v1 (Vint n2))
      | _ => None
      end
  | cmp_case_lp =>
      match v1 with
      | Vlong n1 =>
          let n1 := Int.repr (Int64.unsigned n1) in
          option_map Val.of_bool (Val.cmpu_bool (valid_pointer m) c (Vint n1) v2)
      | _ => None
      end
  | cmp_default =>
      sem_binarith
...
Proof.
  unfold sem_notbool.
  destruct (classify_bool ty); auto.
  destruct v; auto.
  simpl. discriminate.
Qed.
...
Proof.
  unfold sem_cmp.
  destruct (classify_cmp t1 t2); auto.
