...
Definition massert_imp (P Q: massert) : Prop :=
  ...
  (forall m, m_pred P m -> m_pred Q m) /\ (forall al ofs, m_footprint Q al ofs -> m_footprint P al ofs).
...
Definition disjoint_footprint (P Q: massert) : Prop :=
  forall al ofs, m_footprint P al ofs -> m_footprint Q al ofs -> False.

Program Definition sepconj (P Q: massert) : massert := {|
  m_pred := fun m => m_pred P m /\ m_pred Q m /\ disjoint_footprint P Q;
  m_footprint := fun al ofs => m_footprint P al ofs \/ m_footprint Q al ofs
  ;
...
- intuition auto. red; intros. apply (H2 al ofs); auto. 
- intuition auto.
Qed.
...
Program Definition range (b: Mem.abs_block) (lo hi: Z) : massert := {|
  m_pred := fun m =>
...
- red; simpl. intros. destruct H3. subst al.
...
+ omega.
+ apply H5; omega.
+ red; simpl; intros. intuition omega. 
- intuition omega. 
Qed.
...
Proof.
  ...
  assert (H: Mem.valid_access m chunk (MemBlock b) ofs Writable) by eauto with mem.
...
- apply (m_invar P) with m; auto. 
  ...
  + eapply Mem.store_strong_unchanged_on; eauto.
    intros; red; intros. apply (C (MemBlock b) i); simpl; auto.
...
Program Definition minjection (j: meminj) (m0: mem) : massert := {|
  ...
  m_footprint := Mem.block_footprint (fun b ofs => exists b0 delta, j b0 = Some(b, delta) /\ Mem.perm m0 (MemBlock b0) (ofs - delta) Max Nonempty);
  m_invar_weak := true
|}.
Next Obligation.
...
Proof.
  ...
  assert (VALID: Mem.valid_access m1 chunk (MemBlock b1) (Ptrofs.unsigned ofs1) Writable)
    by eauto with mem.
...
- apply (m_invar P) with m2; auto.
  ...
  apply V1. omega.
- red; simpl; intros. destruct al; simpl in *; auto. destruct H1 as (b0 & delta0 & U & V).
...
Lemma alloc_parallel_rule:
  ...
  0 <= delta -> hi <= sz2 ->
  exists j',
     m2' |= range (MemBlock b2) 0 lo ** range (MemBlock b2) hi sz2 ** minjection j' m1' ** P
  /\ inject_incr j j'
  /\ j' b1 = Some(b2, delta)
...
* apply Mem.perm_implies with Freeable; auto with mem.
  eapply Mem.perm_alloc_2; eauto. omega. 
* red; simpl; intros. intuition subst.  omega. 
* red; simpl; intros.
  simpl in *; auto.
  assert (al = MemBlock b2) by tauto. subst al. 
  assert (0 <= ofs < lo \/ hi <= ofs < sz2) by tauto. clear H1.
  destruct H2 as (b0 & delta0 & D & E).
  eapply Mem.perm_alloc_inv in E; eauto. 
  destruct (Mem.eq_abs_block (MemBlock b0) (MemBlock b1)).
 inv e. rewrite J2 in D. inversion D; clear D; subst delta0. xomega.
  rewrite J3 in D by congruence. elim FRESH2. eapply Mem.valid_block_inject_2; eauto. 
+ apply (m_invar P) with m2; auto.
  cut (Mem.strong_unchanged_on (m_footprint P) m2 m2').
  {
    destruct (m_invar_weak P); auto using Mem.strong_unchanged_on_weak.
...
* eelim DISJ; eauto. simpl. 
  ...
  destruct (Mem.eq_abs_block (MemBlock b0) (MemBlock b1)).
  inv e. rewrite J2 in A. inversion A; clear A; subst b delta0. contradiction.
  rewrite J3 in A by congruence. exists b0, delta0; auto.
Qed.
...
Proof.
  ...
  simpl in E.
  assert (PERM: Mem.range_perm m2 (MemBlock b2) 0 sz2 Cur Freeable).
  { red; intros. 
...
- simpl. eapply Mem.free_right_inject; eauto. eapply Mem.free_left_inject; eauto.
  intros. apply (F (MemBlock b2) (ofs + delta0)).
+ simpl.
...
- red; simpl; intros. eelim C; eauto. 
  simpl. right. destruct al; simpl in *. auto.
...
Lemma alloc_parallel_rule_2:
  ...
  0 <= delta -> hi <= sz2 ->
  exists j',
     m2' |= range (MemBlock b2) 0 lo ** range (MemBlock b2) hi sz2 ** minjection j' m1' ** globalenv_inject ge j' ** P
  /\ inject_incr j j'
  /\ j' b1 = Some(b2, delta)
  /\ inject_separated j j' m1 m2 .
Proof.
