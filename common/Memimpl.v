...
Lemma mkmem_ext:
 forall cont1 cont2 acc1 acc2 next1 next2 a1 a2 b1 b2 c1 c2 adt1 adt2 sv1 sv2,
  cont1=cont2 -> acc1=acc2 -> next1=next2 -> adt1 = adt2 ->
  mkmem cont1 acc1 next1 a1 b1 c1 adt1 sv1 = mkmem cont2 acc2 next2 a2 b2 c2 adt2 sv2.
Proof.
...
Lemma non_private_stack_access_dec : forall m b lo hi,
  ...
  {~non_private_stack_access m b lo hi}.
Proof.
  intros. unfold non_private_stack_access, strong_non_private_stack_access.
  destruct (is_stack_top_dec m b); auto.
  destruct (get_frame_info m b); auto.
  destruct (in_stack_data_dec lo hi f); auto.
  right. intuition. 
Qed.
...
Program Definition unchecked_free (m: mem) (b: block) (lo hi: Z): mem :=
  mkmem m.(mem_contents)
        (PMap.set b
                ...
                m.(mem_access))
        m.(nextblock) _ _ _ (stack_adt m) _.
Next Obligation.
  easy.
Qed.
Next Obligation.
...
Proof.
  ...
  intuition.
  right. eapply in_stack_data_concat; eauto.
Qed.
...
Proof.
  ...
  red; intros. exploit storebytes_non_private_stack_access_3; eauto. rewrite app_length.
  rewrite inj_plus. unfold non_private_stack_access, strong_non_private_stack_access.
  destruct (get_frame_info m b); auto.
  intros [A | A]; auto. right.
  red; intros; apply A. omega.
  ...
  red; intros. exploit storebytes_non_private_stack_access_3. apply H.
  rewrite app_length.
  rewrite inj_plus. unfold non_private_stack_access, strong_non_private_stack_access.
  rewrite (storebytes_get_frame_info _ _ _ _ _ ST1).
  destruct (get_frame_info m b); auto.
  intros [A | A]; auto.
  left.
  eapply storebytes_is_stack_top; eauto.
  right. red; intros; apply A. omega.
...
Proof.
  ...
  destruct (get_frame_info m1 b'); auto. rewrite alloc_is_stack_top.
  destruct C; auto.
Qed.
...
Proof.
  ...
  split; auto.
  red; intros. right; red. rewrite alloc_get_frame_info_new; auto.
Qed.
...
Proof.
  ...
  unfold non_private_stack_access, strong_non_private_stack_access in *; intros.
  rewrite <- alloc_get_frame_info.
  destruct (get_frame_info m2 b'); auto. rewrite <- alloc_is_stack_top.
  destruct C; auto.
Qed.
...
Proof.
  ...
  rewrite <- H in IST. simpl in *.
  inv H1; simpl in *.
  - destruct IST; try easy. subst. left. apply FB0 in FB. intuition.
  - rewrite <- INJlist; eauto.
  - left; symmetry. rewrite <- INJlist; eauto.
  - destruct IST; try easy. subst. left.
...
Proof.
  ...
  - constructor.
  - inv H.
    + destruct (eq_block b1 b3).
      * subst. assert (b2 = b4). eapply FB0. eauto. subst.
        destruct (eq_block b4 b4); try congruence.
        assert (delta = 0) by (eapply FB0; eauto). subst.
        destruct fi; econstructor.
      * destruct (eq_block b2 b4); auto.
...
Proof.
  ...
  generalize (get_frame_info_inj _ _ _ _ _ _ MINJ FB).
  inversion 1.
  - (* None -> None *)
    auto.
  - (* Some a -> Some a*)
...
Lemma in_frame_with_info_in_frames:
  forall b fi l,
    In (frame_with_info b fi) l -> in_frames l b.
Proof.
...
Proof.
  destruct a; constructor; inversion 1; tauto.
Qed.
...
Proof.
  ...
  inv MINJ.
  rewrite <- H0 in IST. simpl in *; easy.
  rewrite <- H in IST. unfold inject_id in *; simpl in *.
  inv H1; simpl in *.
  - specialize (FB _ _ eq_refl). destruct FB; subst. destruct IST; try easy. subst. left; congruence.
  - rewrite <- INJlist; eauto.
  - left; symmetry. rewrite <- INJlist; eauto.
  - destruct IST; try easy. subst. left.
...
Proof.
  ...
  - constructor.
  - inv H.
    + specialize (FB _ _ eq_refl). destruct FB. subst. 
      destruct (eq_block b b2); try congruence.
      subst.
      destruct fi; econstructor.
    ...
    + destruct (in_dec eq_block b bl0).
      * specialize (INJlist b _ _ eq_refl).
        ...
        destruct ofi; constructor.
        subst.
        intros; replace ofs with (ofs + 0) by omega.
        eapply INDATA; eauto.
      * destruct (eq_block b b2); try intuition congruence.
        rewrite <- INJlist in e; eauto. intuition congruence.
    +
      specialize (INJ b0 _ _ eq_refl). 
      destruct (eq_block b b0). subst.
      destruct (eq_block b0 b2); try intuition congruence. subst.
      econstructor. intros. eapply INDATA in H. rewrite Z.add_0_r in H. auto.
      auto.
      destruct (eq_block b b2); try intuition congruence.
Qed.
...
Lemma non_private_stack_access_magree:
  forall P m1 m2 b lo hi p
    ...
    non_private_stack_access m2 b lo hi.
Proof.
  unfold non_private_stack_access, strong_non_private_stack_access. intros P m1 m2 b lo hi p MINJ RP.
  generalize (get_frame_info_magree _ _ _ b MINJ).
  inversion 1.
  - (* None -> None *)
    auto.
  - (* Some a -> Some a*)
    intros [C|C].
    + left.
      eapply is_stack_top_magree; eauto.
    + auto.
  - intros _.
    right.
    red. intros. eapply H3.
    apply RP. omega.
Qed.
...
Lemma frame_inject_incr:
  forall f f' m f1 f2 (* thr thr' *),
    inject_incr f f' ->
    (* (forall b, in_frame f1 b -> Plt b thr ) -> *)
    (* (forall b, in_frame f2 b -> Plt b thr' ) -> *)
...
Proof.
  ...
  - econstructor.
    ...
    intros.
    destruct (f b') as [[b delta0]|] eqn: EQ.
    exploit INCR. rewrite EQ. eauto. rewrite H1. inversion 1; subst.
    eapply INJ. eauto.
    specialize (NEW _ _ _ EQ H1).
    destruct NEW  as (A & B & C).
...
Proof.
  ...
  assert (mem_inj f' m1 m2).
    ...
    eapply list_forall2_imply. eauto.
    intros. eapply frame_inject_incr. eauto.
    unfold f'. intros. destruct (eq_block b b1); try congruence.
    auto.
...
Proof.
  inversion 1; intros IFF FBB; simpl in *; 
    try first [
        intuition congruence
        | rewrite <- INJlist; eauto ].
  subst. simpl in *. eapply FB; eauto.
  subst; simpl in *. eapply INJ in FBB. intuition.
Qed.
...
Lemma zero_delta_inject f m1 m2:
  ...
  Mem.inject f m1 m2.
Proof.
  intros H H0 NODUP H1 H1INV H2 STACK.
  constructor.
  {
    constructor.
...
Proof.
  ...
  destruct a.
  constructor.
  intros. destruct (SELF b); try congruence. rewrite H in H0; inv H0. auto. 
  intros. destruct (SELF b'); congruence.
  constructor. intros. destruct (SELF b). congruence.
...
Proof.
  ...
  + intros b1 b2 H2 o v1 H3.
    ...
    reflexivity.
  + apply self_inject_list_frames; auto.
Qed.
...
Proof.
  ...
  - econstructor; eauto.
    + intros b' delta A. repeat destr_in A.
      autospe. intuition.
    + intros b' delta A. repeat destr_in A.
      autospe. intuition.
  - econstructor; eauto. intros; autospe.
    eapply INDATA. replace ofs with (ofs + 0). eapply PERM; eauto. omega.
    intros; autospe.
    split; intros.
    + autospe. intuition. 
    + autospe. intuition subst. autospe. auto.
  ...
  - econstructor; eauto. intros; autospe.
    ...
    intros; autospe. rewrite INJlist. intuition subst; autospe; auto.
  - econstructor; eauto.
    + intros; autospe.
      assert (b = b2) by intuition. subst.
      replace (ofs + (z + z0)) with ((ofs + z) + z0) by omega.
      eapply INDATA0; eauto. 
    + intros; autospe. intuition.
  - econstructor; eauto. intros; autospe.
    rewrite Z.add_0_r. eapply INDATA; eauto.
    intros; autospe. rewrite INJ. intuition subst; autospe; auto.
Qed.
...
Proof.
  ...
  (* stack adt *)
  eapply list_frame_inject_compose; eauto.
Qed.
...
Lemma push_frame_perm_1:
  forall m1 f m2 b (PF: push_frame m1 f = Some (m2, b)),
  forall b' o k p, perm m1 b' o k p -> perm m2 b' o k p.
Proof.
  unfold push_frame. intros.
  autospe.
  rewrite add_adt_perm.
  eapply perm_alloc_1; eauto. eauto.
Qed.

Lemma push_frame_valid_block_1:
   forall m1 f m2 b (PF: push_frame m1 f = Some (m2, b)),
     forall b', valid_block m1 b' -> valid_block m2 b'.
Proof.
  unfold push_frame. intros.
  autospe.
  rewrite add_adt_valid_block.
  eapply valid_block_alloc; eauto. eauto.
Qed.

Lemma push_frame_valid_new_block:
   forall m1 f m2 b (PF: push_frame m1 f = Some (m2, b)),
     valid_block m2 b.
Proof.
  unfold push_frame. intros.
  autospe.
  rewrite add_adt_valid_block.
  eapply valid_new_block; eauto. eauto.
Qed.

Lemma push_frame_fresh_block:
   forall m1 f m2 b (PF: push_frame m1 f = Some (m2, b)),
     ~ valid_block m1 b.
Proof.
  unfold push_frame. intros.
...
Lemma push_frame_is_stack_top:
   forall m f m' sp,
     Mem.push_frame m f = Some (m', sp) ->
     Mem.is_stack_top m' sp.
Proof.
  unfold push_frame, add_adt. intros.
  autospe. unfold is_stack_top, get_stack_top_blocks. simpl. auto.
Qed.

Lemma push_frame_succeeds:
   forall m1 f,
   exists m2 b,
     Mem.push_frame m1 f = Some (m2, b).
Proof.
  unfold push_frame. intros.
  destruct (alloc m1 0 (frame_size f)) eqn:heqo.
  destruct (add_adt m (frame_with_info b (Some f))) eqn:ADT. eauto.
  unfold add_adt in ADT; autospe. exfalso; apply n.
...
Lemma push_frame_strong_unchanged_on:
   forall P m f m' b,
...
Lemma push_frame_stack_blocks:
   forall m1 f m2 b,
     push_frame m1 f = Some (m2, b) ->
     stack_adt m2 = (frame_with_info b (Some f)) :: stack_adt m1.
Proof.
  unfold push_frame, add_adt; intros; autospe. simpl.
...
Lemma not_in_frames_frame_inject:
  forall b b' delta f (FB: f b = Some (b', delta)) m1 l l0, list_forall2 (frame_inject f m1) l l0 -> ~ in_frames l b -> ~ in_frames l0 b'.
Proof.
  induction 2; simpl; intros; eauto. 
  destruct (in_frame_dec a1 b); auto.
  destruct (in_frames_dec al b); auto.
  intro A.
  inv H; simpl in *; intros; eauto; autospe; intuition try congruence.
  apply H3. eapply INJ. subst; eauto.
Qed.
...
Proof.
  ...
  - change (perm m1 b0 ofs k p) in H0.
...
Proof.
  ...
  eapply unrecord_stack_block_perm'. eauto.
  unfold free_top_frame in Heqo0.
  autospe. unfold is_stack_top, get_stack_top_blocks in H1.
  rewrite Heql in H1. simpl in H1. eapply perm_free_1; eauto.
Qed.

Lemma nextblock_free_top_frame:
   forall m1 m2,
     free_top_frame m1 = Some m2 ->
     nextblock m2 = nextblock m1.
Proof.
  unfold free_top_frame; intros; autospe.
  eapply nextblock_free; eauto.
Qed.

Lemma nextblock_pop_frame:
   forall m1 m2,
     pop_frame m1 = Some m2 ->
     nextblock m2 = nextblock m1.
Proof.
  unfold pop_frame; intros; autospe.
  rewrite (unrecord_stack_block_nextblock _ _ H).
  eapply nextblock_free_top_frame; eauto.
Qed.

Lemma free_top_frame_valid_block:
   forall m1 m2,
     free_top_frame m1 = Some m2 ->
     forall b, valid_block m1 b -> valid_block m2 b.
Proof.
...
Lemma pop_frame_valid_block:
   forall m1 m2,
     pop_frame m1 = Some m2 ->
     forall b, valid_block m1 b -> valid_block m2 b.
Proof.
  unfold pop_frame; intros; autospe.
  eapply unrecord_stack_block_valid_block. eauto.
  eapply free_top_frame_valid_block; eauto.
Qed.

Lemma free_top_frame_stack_adt:
   forall m1 m2,
     free_top_frame m1 = Some m2 ->
     stack_adt m2 = stack_adt m1.
Proof.
...
Lemma pop_frame_stack_blocks_1:
   forall m1 m2 b r,
     pop_frame m1 = Some m2 ->
     stack_adt m1 = b :: r ->
     stack_adt m2 = r.
Proof.
  unfold pop_frame; intros; autospe.
  edestruct unrecord_stack_adt; eauto.
  rewrite (free_top_frame_stack_adt _ _ Heqo) in H1.
  rewrite H0 in H1; inv H1. auto.
Qed.

Lemma pop_frame_stack_blocks_2:
   forall m1 m2 r,
     pop_frame m1 = Some m2 ->
     stack_adt m2 = r ->
...
Proof.
  unfold pop_frame; intros; autospe.
  rewrite <- (free_top_frame_stack_adt _ _ Heqo).
  unfold_unrecord. simpl.
  rewrite H0; simpl; eauto.
Qed.

Lemma pop_frame_get_frame_info:
   forall m1 m2 b,
   pop_frame m1 = Some m2 ->
   ~ is_stack_top m1 b ->
   get_frame_info m2 b = get_frame_info m1 b.
Proof.
  unfold get_frame_info, is_stack_top, get_stack_top_blocks.
  intros.
  exploit pop_frame_stack_blocks_2; eauto. intros (b0 & EQ). rewrite EQ in *.
  simpl.
  destruct b0; simpl in *; 
  rewrite pred_dec_false; eauto.
Qed.

Lemma load_pop_frame :
   forall (m1 m2 : mem),
     pop_frame m1 = Some m2 ->
...
Proof.
  unfold pop_frame. intros. autospe.
  transitivity (load chunk m b ofs).
  unfold_unrecord.
  unfold load. simpl.
  destruct (valid_access_dec m chunk b ofs Readable),
  (valid_access_dec
      ...
      access_max := access_max m;
      nextblock_noaccess := nextblock_noaccess m;
      contents_default := contents_default m;
      stack_adt := tl (stack_adt m);
      stack_valid := fun (b0 : block) (pf : in_frames (tl (stack_adt m)) b0) => stack_valid m b0 (in_frames_tl (stack_adt m) b0 pf) |}
      chunk b ofs Readable); auto.
  exfalso; apply n. destruct v. destruct H2.
  repeat split; eauto.  inversion 1.
  exfalso; apply n. destruct v. destruct H2.
  repeat split; eauto.  inversion 1.
  unfold free_top_frame in Heqo. autospe.
  eapply load_free. eauto. left.
  unfold is_stack_top, get_stack_top_blocks in H0; rewrite Heql in H0. simpl in *; intuition.
Qed.

Lemma nextblock_push_frame:
   forall m1 f m2 b,
     push_frame m1 f = Some (m2, b) ->
     nextblock m2 = Pos.succ (nextblock m1).
Proof.
  unfold push_frame; intros; autospe.
  erewrite add_adt_nextblock; eauto.
  eapply nextblock_alloc; eauto.
Qed.

Lemma push_frame_result:
   forall m1 f m2 b,
     push_frame m1 f = Some (m2, b) ->
     b = nextblock m1.
Proof.
  unfold push_frame; intros; autospe.
  eapply alloc_result; eauto.  
Qed.

Lemma push_frame_get_frame_info:
...
Proof.
  unfold get_frame_info. intros.
  erewrite push_frame_stack_blocks; eauto.
  simpl. auto.
Qed.
...
Lemma free_top_frame_parallel_extends:
   forall m1 m2 m1' b fi r,
     extends m1 m2 ->
     Mem.stack_adt m1 = frame_with_info b (Some fi) :: r ->
      free_top_frame m1 = Some m1' ->
      exists m2',
        free_top_frame m2 = Some m2'
        /\ extends m1' m2'.
Proof.
  intros.
  inversion H. inversion mext_inj0.
  unfold free_top_frame in *; inv mi_stack_blocks0; intros; autospe.
  inv H2. inv H4.
  exploit free_parallel_extends; eauto.
  replace b0 with b3; auto.
  eapply INJ. reflexivity.
Qed.
...
Lemma record_stack_block_extends:
  forall m1 m2 m1' b fi,
    ...
    exists m2',
      ...
      extends m1' m2'.
Proof.
  unfold record_stack_block, add_adt.
  intros; autospe. simpl in *.
  destruct (valid_frame_dec (frame_with_info b fi) m2).
  - eexists; split; eauto.
    inv H.
    constructor; simpl; intros; eauto.
    ...
    + eapply mi_perm0; eauto.
    + constructor; auto.
      * unfold inject_id; constructor; simpl; inversion 1; auto.
      * eapply list_forall2_frame_inject_invariant. 2: eauto.
        unfold inject_id. inversion 1; subst. tauto.
  - exfalso.
    apply n.
    eapply valid_frame_extends; eauto.
Qed.
...
Lemma pop_frame_parallel_extends:
   forall m1 m2 m1' b fi r,
     extends m1 m2 ->
     Mem.stack_adt m1 = frame_with_info b (Some fi) :: r ->
...
Proof.
  unfold pop_frame; intros; autospe.
  exploit free_top_frame_parallel_extends; eauto.
  intros (m2' & A & B); rewrite A.
  exploit unrecord_stack_block_extends; eauto.
Qed.
 
Lemma add_adt_extends:
    forall m1 m2 fi m1',
      extends m1 m2 ->
      add_adt m1 fi = Some m1' ->
      exists m2',
        add_adt m2 fi = Some m2'
        /\ extends m1' m2'.
Proof.
  unfold add_adt; intros; autospe.
  ...
  - eapply mi_perm0; eauto.
  - constructor.
    + apply self_inject_frame. right; reflexivity.
    + eapply list_forall2_frame_inject_invariant. 2: eauto.
      unfold inject_id; inversion 1; subst. tauto.
Qed.

Lemma push_frame_extends:
...
Proof.
  unfold push_frame; intros; autospe.
  exploit alloc_extends; eauto. apply Zle_refl. apply Zle_refl.
  intros (m2' & A & B). rewrite A.
  exploit add_adt_extends; eauto. intros (m2'' & C & D); rewrite C.
  eauto.
Qed.
...
Proof.
  split; unfold push_frame; intros; autospe. eauto.
  destruct H as (m1 & ALLOC & RSB).
  rewrite ALLOC.
  unfold record_stack_block in RSB.
  rewrite RSB. auto.
Qed.

Lemma perm_push_frame_2:
   forall m1 fi m2 b,
...
Proof.
  unfold record_stack_block, add_adt.
  intros m1 m1' m2 j fi1 fi2 b b' delta JB INJ FI RSB.
  autospe.
  ...
  - eexists; split; eauto.
    ...
    + inv mi_inj0; constructor; simpl; intros; eauto.
      * eapply mi_perm0; eauto.
      * constructor. 
        eapply frame_inject_invariant. 2: eauto. tauto.
...
Proof.
  unfold pop_frame, free_top_frame; intros.
  rewrite H.
  split; intros; autospe; eauto.
  destruct H0 as (m1 & A & B); rewrite A. auto.
Qed.
...
Lemma record_stack_blocks_inject_into_one:
  forall j m1 m2 bl b m1'
    ...
    (RSB:  record_stack_blocks m1 bl = Some m1')
    (VALID: valid_block m2 b),
    exists m2',
      record_stack_block m2 b None = Some m2' /\
      inject j m1' m2'.
Proof.
  unfold record_stack_blocks, record_stack_block, add_adt.
  intros; autospe.
  clear Heqs.
...
Proof.
  unfold record_stack_blocks, record_stack_block, add_adt.
  intros j m1 m2 bl bl' m1' INJ FOR RSB VALID; autospe.
  clear Heqs.
...
Proof.
  ...
  exact push_frame_perm_1.
  exact push_frame_valid_block_1.
  exact push_frame_valid_new_block.
  exact push_frame_fresh_block.
  exact push_frame_is_stack_top.
  exact push_frame_succeeds.
  exact push_frame_strong_unchanged_on.
  exact store_stack_blocks.
  exact is_stack_top_stack_blocks.
  exact push_frame_stack_blocks.
  exact perm_pop_frame.
  exact nextblock_pop_frame.
  exact pop_frame_valid_block.
  exact pop_frame_stack_blocks_1.
  exact pop_frame_stack_blocks_2.
  exact pop_frame_get_frame_info.
  exact load_pop_frame.
  exact nextblock_push_frame.
  exact push_frame_result.
  exact push_frame_get_frame_info.
