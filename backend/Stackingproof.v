...
Lemma perm_free m b lo hi m':
  ...
  forall b' o' k p,
    Mem.perm m' b' o' k p <->
    ((~ (b' = b /\ lo <= o' < hi)) /\ Mem.perm m b' o' k p).
Proof.
  intros H b' o' k p.
...
Proof.
  ...
  inv EQ.
  erewrite perm_free by eassumption.
  erewrite perm_free in PERM' by eassumption.
...
Proof.
  intros EC j m m' MPG K IMAGE MEXT MINJ IAOOB ll lm sg MS args' INJARGS vl' m'2.
  generalize (Mem.extends_inject_compose _ _ _ _ MEXT MINJ).
  intros MINJ'.
  exploit external_call_mem_inject ; eauto.
  intros [w Hw].
  assert (Ht: match_events ge cc_inject w t t) by admit. (* XXX WRONG *)
  specialize (Hw t Ht).
  destruct Hw as (_ & res'_ & m'2_ & EC2 & _ & _ & _ & UNCHANGED & _ & _).
  intros EC2' IAIB.
...
Proof.
  ...
  rewrite EQ in H2.
  edestruct (Mem.range_perm_free m'0 sp' of (of + size_chunk (chunk_of_type ty))) as (m2 & Hm2).
  {
    red.
    intros ofs H3.
  ...
  + revert Hno_init_args.
    ...
    * eapply frame_undef_regs; eauto.
      ...
      -- simpl. red; simpl. intros sl of ty IN rs2.
         ...
         assert (b0 = b1).
         clear - H8 e INJ_INIT_SP HAMOA SP_NOT_INIT. subst.
         eapply HAMOA; eauto.
...
- (* Lbuiltin *)
  ...
  rename SEP into SEP_init; destruct Hw as (j' & res' & m1' & EC & RES & SEP & INCR & ISEP).
  econstructor; split.
  + apply plus_one. econstructor; eauto.
  ...
  + constr_match_states.
    ...
    * apply frame_set_res. apply frame_undef_regs. apply frame_contents_incr with j; auto.
      ...
      -- simpl.
         ...
         admit. (* XXX MORE EXTERNAL_CALL_MEM_XXX *) (*
         ...
         revert HAMOA INJ_INIT_SP UNCH3 SEP_init IAOOB INCR.
         red; intros.
         exploit SEP_init. eauto. instantiate (1 := rs2).
         intros (v & ea & inj); eexists; split; eauto.
         ...
         red; red; intros.
         exploit IAOOB; eauto.
         exploit HAMOA. eauto. apply INJ_INIT_SP. apply H8.
         intro. subst b0.
         assert (delta = 0) by congruence. subst delta.
         rewrite Zminus_0_r in H10; eauto.
         *)
  ...
  + constr_match_states.
    ...
    * revert Hno_init_args.
      ...
      eapply init_args_out_of_bounds_alloc. eauto.
      {
        revert ISP'VALID.
        apply mconj_proj1 in SEP_init; apply sep_proj1 in SEP_init; simpl in SEP_init ; eauto.
        clear - m_ext SEP_init INJ_INIT_SP.
    ...
    * apply mconj_proj1 in SEP_init; apply sep_proj1 in SEP_init; simpl in SEP_init ; eauto.
      revert SEP_init INJ_INIT_SP A . clear.
    ...
    *
      ...
      -- simpl.
         ...
         revert ISP'VALID A. clear. destruct init_sp; simpl; try congruence.
         intros.
...
- (* external function *)
  ...
  * revert Hsome_init_args Hno_init_args.
    ...
    -- inv f_eq. eauto.
       admit. (* XXX MORE EXTERNAL_CALL_MEM_FOOBAR *)
  ...
  * apply stack_contents_change_meminj with j; auto.
    ...
    -- simpl.
       ...
       revert HAMOA INJ_INIT_SP UNCH3 IAM Hno_init_args D. clear.
       red; intros.
       exploit IAM. eauto. instantiate (1 := rs).
       intros (v & ea & inj); eexists; split; eauto.
       inv ea; constructor; eauto.
...
Theorem transf_program_correct':
  forward_simulation cc_inject (Linear2.semantics prog) (Mach.semantics return_address_offset tprog).
Proof.
...
Theorem transf_program_correct:
  forward_simulation cc_inject (Linear.semantics prog) (Mach.semantics return_address_offset tprog).
Admitted. (* Need compose_forward_simulations *)
(*
Proof.
