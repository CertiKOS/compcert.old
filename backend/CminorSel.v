...
Inductive eval_expr: letenv -> expr -> val -> Prop :=
  ...
  | eval_Eop: forall le op al vl v,
      ...
      eval_operation ge op vl m = Some v ->
      eval_expr le (Eop op al) v
  | eval_Eload: forall le chunk addr al vl vaddr v,
      eval_exprlist le al vl ->
      eval_addressing ge addr vl = Some vaddr ->
...
Inductive step: state -> trace -> state -> Prop :=
  ...
  | step_store: forall f chunk addr al b k sp e m vl v vaddr m',
      eval_exprlist e m nil al vl ->
      eval_expr e m nil b v ->
      eval_addressing ge addr vl = Some vaddr ->
  ...
  | step_call: forall f optid sig a bl k sp e m vf vargs fd,
      eval_expr_or_symbol e m nil a vf ->
      eval_exprlist e m nil bl vargs ->
...
Lemma eval_lift_expr:
  forall ge e m w le a v,
  eval_expr ge e m le a v ->
  forall p le', insert_lenv le p w le' ->
  eval_expr ge e m le' (lift_expr p a) v.
Proof.
  intros until w.
  apply (eval_expr_ind3 ge e m
    (fun le a v =>
      forall p le', insert_lenv le p w le' ->
      eval_expr ge e m le' (lift_expr p a) v)
    (fun le al vl =>
      forall p le', insert_lenv le p w le' ->
      eval_exprlist ge e m le' (lift_exprlist p al) vl)
    (fun le a b =>
      forall p le', insert_lenv le p w le' ->
      eval_condexpr ge e m le' (lift_condexpr p a) b));
...
Lemma eval_lift:
  forall ge e m le a v w,
  eval_expr ge e m le a v ->
  eval_expr ge e m (w::le) (lift a) v.
Proof.
